    ...
    <div class="modal" v-if="step==0">
        ...
        <div class="modal-body">
            ...
            <button style="cursor: pointer; overflow: hidden; text-align: center;">
              <input type="file" accept=".png, .jpeg, .jpg, .gif" @change="selectFile" class="custom-file-input3" style="border: none; background: #FE6637; color: white; width: 100%; height: 100%;padding: 7px 26px;">
            </button>
        </div>
...
//updateAvatar
        ...
        getProfileCoverModer(){
            ApiWrapper.getModerCover({id: this.idModeration})
                .then(async el => {
                  ...
                  if(status == 1){
                    ...
                    await ApiWrapper.deleteCoverByUser({id: this.idModeration});
                    this.step=2;
                    return;
                  }
                  if(status == 2){
